...
export default function DailyMixPage() {
  ...
  return (
    <PageWrapper>
      <h1 className="text-2xl font-bold text-gray-900 mb-2">✨ Tagesmix</h1>
          ...
          <div className="flex flex-col gap-3">
            {reviewTopics.map((topic) => topic && (
              <Link key={topic.id} href={`/learn/${topic.id}`}>
                <Card padding="sm" className="hover:shadow-md transition-shadow cursor-pointer border-2 border-amber-200">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-col gap-3">
            {newTopics.map((topic) => topic && (
              <Link key={topic.id} href={`/learn/${topic.id}`}>
                <Card padding="sm" className="hover:shadow-md transition-shadow cursor-pointer border-2 border-blue-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-gray-900">{topic.title}</p>
      ...
      {reviewTopics.length === 0 && newTopics.length === 0 && (
        <Card className="text-center">
          ...
          <h2 className="font-bold text-gray-900">Alles für heute erledigt!</h2>
